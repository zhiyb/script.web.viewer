...
try:
    ...
    KODI_VERSION_MAJOR = 16

if KODI_VERSION_MAJOR > 16:
    ACTION_RUN_IN_MAIN = 69
    TRIGGER_RUN_IN_MAIN = 'Action(PlayerProcessInfo)'
else:
    #Actually it's show codec info but I'm using in a threaded callback
    ACTION_RUN_IN_MAIN = 27
    TRIGGER_RUN_IN_MAIN = 'Action(codecinfo)'
...
class ThreadWindow:
    ...
    def runInMain(self, function, *args, **kwargs):
        ...
        xbmc.executebuiltin(TRIGGER_RUN_IN_MAIN)
...
class ViewerWindow(BaseWindow):
    ...
    def doMenu(self, etype=None):
        ...
        d = xbmcDialogSelect(T(32110))

        if StreamExtractor.isDownloading():
            d.addItem('manage_downloads','Manage Downloads')
        d.addItem('goto_url',T(32131))
        d.addItem('bookmarks',T(32132))
        d.addItem('settings',T(32110))
        d.addItem('help',T(32146))
        d.addItem('google_search',T(32149))
        d.addItem('page_search',T(32153))

        if self.simpleControls:
            for ID,label in zip(['back','forward','refresh','history','close'], [T(32158), T(32159), T(32160), T(32112),T(32161)]):
                d.addItem(ID,label)
        elementURL = element and element.fullURL() or ''
        if etype == PE_LINK:
            d.addItem('open_link',T(32134))
            d.addItem('save_link',T(32135))
            if element.image:
                d.addItem('link_image',T(32136))
            if element.isImage():
                d.addItem('target_image',T(32137))
        elif etype == PE_IMAGE:
            d.addItem('view_image',T(32138))
            d.addItem('save_image',T(32139))
        elif etype == PE_FORM:
             d.addItem('submit_form',T(32140))
        if StreamExtractor.mightHaveVideo(elementURL):
             d.addItem('download_video','Download Video')

        if self.page and StreamExtractor.mightHaveVideo(self.page.url):
            d.addItem('play_video',T(32163))
            d.addItem('download_page_video','Download Page Video')
        ...
        elif oid == 'manage_downloads': StreamExtractor.manageDownloads()
...
if __name__ == '__main__':
    ...
    if not start_url: start_url = getHome() or 'http://google.com'
    LOG(start_url)
    apath = xbmc.translatePath(__addon__.getAddonInfo('path'))
    if not os.path.exists(os.path.join(apath,'resources','skins','Default','720p','script-webviewer-page.xml')):
        apath = 'Q:\\scripts\\.modules\\script.web.viewer\\' #for XBMC4Xbox when used as a module
